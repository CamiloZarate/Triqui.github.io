...
function campeon() {
    $("#ganador").show();
    $("#alertMensaje").html('Ganador <strong><input class="ganador btn btn-success btn-lg" type="button" value="' + turno + '"></strong>');
    $(".alert").show('slow');
    $("#iniciar").removeClass("disabled");
    $(".pieza").each(function(index) {
        $(this).attr('disabled', true);
...
function CrearComponentesJuego() {
    ...
    for (i = 1; i <= cantidadOpciones; i++) {
        for (j = 1; j <= cantidadOpciones; j++) {
            $("#" + Juego).append("<input class='pieza btn btn- btn-lg' type='button' value='-' id='" + i + j + "'/>");
        }
    ...
    $("#" + Juego).append('<br/>');
    $("#" + Juego).append('<br/>');
    $("#" + Juego).append('<input type="button" id="iniciar" onclick="Reiniciar()" value="Reiniciar" class="btn btn-success btn-lg    disabled"/>');
}